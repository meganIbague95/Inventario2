...
public class TransaccionesDAO implements TransaccionesDAOInterface {
		...
		try {
			...
			while (set.next()) {
				if (tipoTabla == TipoTablaEnum.CATEGORIA) {
					Categoria categoria = new Categoria(set.getInt(1), set.getString(2));
					lista.add(categoria);
				} else if (tipoTabla == TipoTablaEnum.MARCA) {
					Marca marca = new Marca(set.getInt(1), set.getString(2));
					lista.add(marca);
				}  else if (tipoTabla == TipoTablaEnum.ORIGEN) {
					Origen origen = new Origen(set.getInt(1), set.getString(2));
					lista.add(origen);
				} else if (tipoTabla == TipoTablaEnum.TAMANIO) {
					Tamanio tamanio = new Tamanio(set.getInt(1), set.getString(2));
					lista.add(tamanio);
				}
				else if (tipoTabla == TipoTablaEnum.TIPO) {
					TipoProducto tipo = new TipoProducto(set.getInt(1), set.getString(2));
					lista.add(tipo);
				}
	...
	public void eliminarObjeto(TipoTablaEnum tipoTabla,Integer codigo) throws Exception {
...
//						set.getString(4), set.getInt(5), set.getInt(6), perfil, persona);
	...
	public Usuario consultarUsuarioSeguridad(String login) throws Exception {
		...
		try {
			...
			set = ps.executeQuery();
			if (set.next()) {
				Persona persona = new Persona(set.getString(5), set.getInt(6), set.getString(7), set.getString(8), set.getString(9), set.getString(10), set.getString(11), set.getString(12));
				Perfil perfil = new Perfil(set.getInt(13), set.getString(14));
				Date fechaUltimoIngreso =(set.getDate(15)!=null)?new Date(set.getDate(15).getTime()):null;
				usuario = new Usuario(set.getString(1), set.getString(2), set.getString(3), set.getString(4), perfil, persona, fechaUltimoIngreso, set.getInt(16),set.getInt(17));
			}
	...
	public void editarUsuario(Usuario usuario) throws Exception {
		...
		try {
			...
			ps.setDate(2, new java.sql.Date(usuario.getUltimoIngreso().getTime()));
			ps.setInt(3, usuario.getNumeroIngreso());
			ps.setInt(4, usuario.getIntentosFallidos());
			ps.setString(5, usuario.getConectado());
	...
	public Inventario consultarInventario() throws Exception {
		...
		try {
			...
			if (set.next()) {
				...
				Long milis=set.getDate(3).getTime();
	...
	public List<Object> consultarObjetoCompleto(TipoTablaEnum tipoTabla) throws Exception {
		List<Object> lista = null;
		StringBuilder sql = new StringBuilder(ConstantesSQL.CONSULTAR_OBJETO_COMPLETO);
		sql.append(tipoTabla.getNombreColumna()).append(ConstantesSQL.CONSULTAR_OBJETO_NOMBRE).append(tipoTabla.getNombreTabla());
		System.out.println(sql.toString());
		PreparedStatement ps = null;
		Connection conexion = null;
		ResultSet set = null;
		try {
			...
			set = ps.executeQuery();
			lista = new ArrayList<Object>();
			System.out.println("Esto es lo suyo ");
			while (set.next()) {
				if (tipoTabla == TipoTablaEnum.CATEGORIA) {
					...
					System.out.println("Esto es lo suyo "+marca.getNombre());
					lista.add(marca);
				}else if (tipoTabla == TipoTablaEnum.ORIGEN) {
	...
	public Object consultarObjetoId(TipoTablaEnum tipoTabla, Integer identificacion) throws Exception {
		Object objeto = null;
		StringBuilder sql = new StringBuilder(ConstantesSQL.CONSULTAR_OBJETO_COMPLETO);
		sql.append(tipoTabla.getNombreColumna()).append(ConstantesSQL.CONSULTAR_OBJETO_NOMBRE).append(tipoTabla.getNombreTabla()).append(ConstantesSQL.CONSULTAR_ID).append(tipoTabla.getNombreTabla()).append(ConstantesSQL.IGUAL).append(identificacion);
		System.out.println(sql.toString());
		PreparedStatement ps = null;
		Connection conexion = null;
		ResultSet set = null;
